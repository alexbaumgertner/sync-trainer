...
Расходы пишутся после каждой успешной генерации, показываются в карточке
«Бюджет» и **ограничивают генерацию**:

| Переменная | Что делает |
|---|---|
| `TTS_BUDGET_USD` | потолок за всё время; превышение — отказ с 402 |
| `TTS_MONTHLY_LIMIT_USD` | потолок на календарный месяц, сбрасывается первого числа |
| `TTS_MAX_CHARS` | потолок на одну генерацию |

Проверка идёт **до** синтеза и учитывает стоимость самой заявки, иначе лимит
можно было бы перешагнуть последним запросом. Если лимит задан, а прочитать
накопленную сумму не удалось, генерация останавливается с 503: молча
отключившийся предохранитель хуже временной недоступности.

Один и тот же `budgetBlock()` из `lib/budget.ts` вызывают и сервер в `/api/tts`,
и клиент для блокировки кнопки — чтобы две проверки не разъехались.

Зачем это вообще нужно: **Google не умеет останавливать траты на
Text-to-Speech**. В его бюджетах есть Spend cap enforcement, но он
поддерживает только Cloud Run, Cloud Run Functions, Gemini API и Vertex AI.
Для озвучки остаются письма, причём фактические расходы появляются с
задержкой в несколько часов. Так что жёсткий предел есть только здесь.
