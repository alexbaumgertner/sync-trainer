 ...
 *
 * Имя файла — `engagements_team`, а не `engagement_team`: Payload применяет
 * миграции в алфавитном порядке, а подчёркивание меньше буквы «s», и
 * `engagement_team` встал бы РАНЬШЕ `engagements` — то есть таблица создалась
 * бы до той, на которую ссылается. Поймано только проверкой на чистой базе.
 *
