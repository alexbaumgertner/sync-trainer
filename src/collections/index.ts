...
export const Artifacts: CollectionConfig = {
  ...
  fields: [
    ...
    {
      /**
       * Форма волны для проигрывателя: громкость по столбикам, 0–100.
       *
       * Считается один раз при первом показе и лежит здесь, потому что
       * считать её в браузере нельзя — восемнадцать минут звука это около
       * ста мегабайт отсчётов в памяти вкладки.
       */
      name: "peaks",
      type: "json",
      admin: { readOnly: true, description: "Столбики формы волны, 0–100" },
    },
    {
      // Длительность оттуда же. Раньше её считали из битрейта и однажды
      // ошиблись вдвое: у MPEG-2 своя таблица. Расшифровщик даёт точную.
      name: "durationSec",
      type: "number",
      admin: { readOnly: true },
    },
    {
       ...
       *
       * Хранится готовым файлом, а не нашей структурой, намеренно: WebVTT
       * разбирает сам браузер (`<track kind="metadata">`), поэтому разбора
       * в коде нет вовсе, и тот же файл годится любому другому
       * проигрывателю — этот кусок планируется вынуть в отдельный модуль.
       *
       * Снимается только при синтезе: после склейки границы реплик
       * восстановимы лишь приблизительно.
       */
      name: "cuesVtt",
      type: "textarea",
      admin: { readOnly: true, description: "Карта времени в формате WebVTT" },
    },
    {
      /**
       * Привязаны ли границы фраз к паузам в звуке.
       *
       * У карт, снятых до появления привязки, фразы внутри реплики
       * разложены по длине текста, и подсветка уходит вперёд на секунды.
       * Пересчитать их можно без нового синтеза — отметка говорит, что
       * это уже сделано, и второй раз декодировать файл незачем.
       */
      name: "cuesAligned",
      type: "checkbox",
      defaultValue: false,
      admin: { readOnly: true, description: "Границы фраз притянуты к паузам в звуке" },
    },
  ],
};
